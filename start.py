...
name = mss.mss().monitors[monitor]
x1, y1 = name['width'], name['height']
...
def TextToScreen(current_operation,text:str)->str:
    ...
    cv2.imshow('s',img)
    cv2.waitKey(0)
    text = pytesseract.image_to_string(img, config=f'--psm 8 --oem 3 -c tessedit_char_whitelist={text}')
    # print(text)
    return text
...
def miniGameThree(autohotpy):
    ...
    # Обрезаем область для избегания лишних элементов для обработки
    cropped = gray[y + 3:y + h // 2 + 2, x:x + w]  # gray[y:y + h // 2 + 2, x:x + w]
...
def CheckInventory(autohopty)->int:
    ...
    text = pytesseract.image_to_string(gray,config='--psm 6 --oem 3 -c tessedit_char_whitelist=1234567890/')
